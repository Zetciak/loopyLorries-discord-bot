...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('addcoconuts')
		.setDescription('🥥💠 Add user coconuts (mod)')
		.addUserOption((option) =>
			option
				.setName('target')
				.setRequired(true)
				.setDescription("The user you'd like to add coconuts.")
		)
		.addStringOption((option) =>
			option
				...
				.setRequired(true)
				.setDescription(
					"The amount of coconuts you'd like to give. (1-15)"
				)
		),
					...
					async function (err, result, fields) {
						if (err) {
							...
							if (result.length > 0) {
								...
								dbCon.query(
									`INSERT INTO lp_admin_logs (user, log) VALUES ('${interaction.member.user.username}', '/addcoconuts ${user.username} ${count}')`,
								...
								const replyEmbed = new MessageEmbed()
									...
									.setDescription(
										`🥥 **${count} coconut** have been added to the user's account.`
									)
									.setTimestamp()
									.setFooter({
										text: interaction.member.user.username,
