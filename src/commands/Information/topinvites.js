...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('topinvites')
		.setDescription('🔰 Check top invites users on server')
	...
	async execute(interaction, client) {
		...
		if (
			...
			// Database check
			dbCon.query(
				`SELECT * FROM lp_users ORDER BY invited DESC LIMIT ${limit}`,
					...
					if (err) {
						...
						if (result.length > 0) {
							...
							const replyEmbed = new MessageEmbed()
								.setColor('#FEDB25')
								.addFields({
									name: `**Top ${result.length} inviting members**`,
									value: usersString,
								})
								.setTimestamp()
								.setFooter({
