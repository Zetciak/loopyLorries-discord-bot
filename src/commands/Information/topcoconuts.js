...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('topcoconuts')
		.setDescription('🥥 Check top coconuts users on server')
	...
	async execute(interaction, client) {
		...
		if (
			...
			// Database check
			dbCon.query(
				`SELECT * FROM lp_users ORDER BY tickets DESC LIMIT ${limit}`,
					...
					if (err) {
						...
						if (result.length > 0) {
							...
							const replyEmbed = new MessageEmbed()
								.setColor('#d1d1d1')
								.addFields({
									name: `**Top ${result.length} coconuts 🥥**`,
									value: usersString,
								})
								.setTimestamp()
								.setFooter({
