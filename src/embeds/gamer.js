...
async function sendEmbed(interaction, client) {
	...
	const embed = new MessageEmbed()
		...
		.setTitle('🎮⠀Claim role')
		.setDescription(
			`Claim your <@&${process.env.gamerRole}> role to be notified about upcoming gaming sessions!\nCheck <#954769236725743647> for more.`
		);

	//Button creation
	const buttons = new MessageActionRow().addComponents(
		new MessageButton()
			.setCustomId(`claimGamer`)
			.setLabel(`🎮 Claim`)
			.setStyle(`SECONDARY`),
		new MessageButton()
			.setCustomId(`deleteGamer`)
			.setLabel(`❌ Delete`)
			.setStyle(`SECONDARY`)
	);
